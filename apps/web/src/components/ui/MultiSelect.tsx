	...
	return (
		...
		>
			<Command
				...
				className={cn(
					"overflow-visible bg-transparent flex flex-col justify-evenly space-y-2 h-10 !m-0",
					className
				)}
				dir={dir}
				{...props}
	...
	({ className, children, ...props }, ref) => {
		...
		return (
			<div
				ref={ref}
				className={cn(
					"flex flex-nowrap bg-background whitespace-nowrap !h-10 no-scrollbar overflow-x-auto gap-1 p-1 py-2 border border-input rounded-lg !m-0",
					className
				)}
				{...props}
			>
...
const MultiSelectorContent = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
	({ children }, ref) => {
		...
		return (
			<div ref={ref} className={`relative h-0 !m-0`}>
				{open && children}
			</div>
		);
	}
...
>(({ className, children }, ref) => {
	return (
		<CommandList
			ref={ref}
			className={cn(
				"p-2 flex flex-col gap-2 rounded-md scrollbar-thin scrollbar-track-transparent transition-colors scrollbar-thumb-muted-foreground dark:scrollbar-thumb-muted scrollbar-thumb-rounded-lg w-full absolute bg-background shadow-md z-50 border border-muted top-0",
				className
			)}
		>
			{children}
